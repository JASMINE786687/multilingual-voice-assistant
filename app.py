...
# ══════════════════════════════════════════════════════
# TRANSLATION  — free via deep-translator / Google
# Retries once, falls back to English on failure
# ══════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════
# 12-SERVICE DATABASE  (English master copy)
# ══════════════════════════════════════════════════════
SERVICE_DB = {
    "ration": {
...
KEYWORD_MAP = {
    ...
    "scholarship":       ["scholarship","study","student","கல்வி","छात्रवृत्ति","స్కాలర్షిప్","বৃত్তি"],
...
AGENT_KB = {
    ("aadhaar","aadhar","uid"):
        "For Aadhaar Card: visit uidai.gov.in or nearest Aadhaar Seva Kendra. Give 10 fingerprints and iris scan. Download e-Aadhaar from eaadhaar.uidai.gov.in after 90 days. It is completely free.",
    ("ration","food card"):
        "For Ration Card: visit Civil Supplies office, fill Form A, attach Aadhaar and address proof for all family members. Inspector visits in 30 days for verification.",
    ("pension","old age"):
        "Old Age Pension (IGNOAPS): must be 60 or above and BPL card holder. Apply at Village Panchayat or BDO office. Rs 200 to 500 per month after approval.",
    ("voter","election","epic"):
        "Voter ID (EPIC): visit voters.eci.gov.in, fill Form 6, must be 18 or above. BLO verifies your address. Download e-EPIC from portal. Free of cost.",
    ("pan","income tax"):
        "PAN Card: apply at onlineservices.nsdl.com, fill Form 49A, pay Rs 107. e-PAN emailed within 48 hours. Physical card in 15 to 20 days.",
    ("ayushman","health","pmjay","hospital"):
        "Ayushman Bharat: check eligibility at mera.pmjay.gov.in. Covers Rs 5 lakh per year at empanelled hospitals. Completely cashless and free for eligible families.",
    ("driving","licence","license"):
        "Driving Licence: apply at sarathi.parivahan.gov.in. Get Learner Licence first, wait 30 days, then apply for DL and pass practical test. DL issued in 7 days.",
    ("scholarship","study","student"):
        "Scholarships: visit scholarships.gov.in. Register with Aadhaar, choose SC/ST/OBC/Merit scheme, submit before October-November deadline. Amount credited to bank directly.",
    ("birth","born"):
        "Birth Certificate: collect Form 1 from hospital within 21 days. Register at Municipal office for home births. Certificate issued in 7 working days.",
    ("land","patta","chitta"):
        "Land Records (Patta/Chitta): visit eservices.tn.gov.in for Tamil Nadu or your state portal. Download Patta online for Rs 60.",
    ("income","salary"):
        "Income Certificate: visit e-District portal or nearest CSC. Attach salary slips or self-declaration. Fee Rs 10 to 30. Certificate in 15 days.",
    ("caste","community","sc","st","obc"):
        "Caste Certificate: visit e-District portal or nearest CSC. Attach father caste certificate and Aadhaar. Issued by Tahsildar in 15 days.",
    ("document","paper","required"):
        "Common documents needed: Aadhaar Card, Passport-size photos, Address proof (electricity bill or bank passbook), and existing certificates as required for the specific service.",
    ("fee","cost","charge","free","money","rupee"):
        "Fees: Aadhaar free, Voter ID free, Ayushman Card free, PAN Card Rs 107, Land records Rs 60, Birth/Caste/Income/Ration certificates Rs 10 to 50.",
    ("time","day","long","how long","when"):
        "Processing times: Aadhaar 90 days, Birth Certificate 7 days, Caste Certificate 15 days, Income Certificate 15 days, Ration Card 30 days, PAN Card 15 to 20 days, Driving Licence 7 days.",
    ("offline","without internet","no internet","no network"):
        "VAANI works offline! All 12 services are cached after your first visit. You can search and get steps even without internet. The AI agent also works offline.",
    ("language","tamil","hindi","telugu","kannada","malayalam","bengali","marathi"):
        "VAANI supports 8 languages: English, Tamil, Hindi, Telugu, Kannada, Malayalam, Bengali, and Marathi. Select your language from the pills at the top, then speak or type.",
    ("help","what","how","explain","vaani"):
        "VAANI helps you access 12 government services in 8 Indian languages using voice or text. Services: Ration Card, Aadhaar, Pension, Voter ID, Ayushman Health, PAN Card, Driving Licence, Scholarship, Birth Certificate, Land Records, Income Certificate, and Caste Certificate.",
}

def get_agent_answer(question):
    q_lower = question.lower()
    for keywords, answer in AGENT_KB.items():
        if any(kw in q_lower for kw in keywords):
            return answer
    return "I can help with: Ration Card, Aadhaar, Pension, Voter ID, Ayushman Health, PAN Card, Driving Licence, Scholarship, Birth Certificate, Land Records, Income Certificate, and Caste Certificate. Please ask about any of these!"
...
def get_service():
    ...
    return jsonify({
        ...
        "service_key": key,
        "steps_en":    list(svc["steps"]),   # always include English for offline cache
        "name_en":     svc["name"]
    })
...
def agent():
    ...
    if lang != "en":
        answer = safe_translate(answer, lang)
    return jsonify({"answer": answer})

@app.route("/feedback", methods=["POST"])
def feedback():
    data = request.get_json()
    feedback_db.append({"service":data.get("service",""),"rating":data.get("rating",""),"time":str(datetime.now())})
